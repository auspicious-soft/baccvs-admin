...
export default function Page() {
  ...
  const { mutate: submitOtp, isPending: submitPending } = useMutation({
    mutationFn: async (payloadData: { method: string; code: string }) => {
      const data = await api.post(
        type.get("type") !== "forget" ? "verify-otp" : "reset-verify-otp",
        payloadData
      );
      return data;
    },
    onSuccess: async (data) => {
      if (type.get("type") !== "forget") {
        router.push(`/address`);
        setUser({...data.data.data, hasShippingDetails: false});
      } else {
        router.push(`/new-password?token=${data.data.data.token}`);
      }
    },
    onError: (error) => {
  ...
  const handleSubmit = () => {
    ...
    if (code.length === length) {
      submitOtp({ method: email?.get("email") || "", code: code });
    } else {
      ...
      <AuthCard className="gap-2">
        ...
        <span className="my-2">
          ...
          <span
            className="border-b-[1px] cursor-pointer"
            onClick={() =>
              !resendPending && resendOtp({ value: email?.get("email") || "" })
            }
          >
            Resent Otp
          </span>
